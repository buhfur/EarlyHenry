...
class Talk:
    ...
    def speak(self, request):
        ...
        elif request == "Henry read me all the pdfs i have downloaded" or request == "hey what files do i have downloaded" or request == "okay so what pdfs do i have downloaded" or request == "downloaded files" or request == "what pdfs are downloaded":
            self.user_speak.Speak("give me one second.")
            time.wait(2)
            #have henry read off the pdf file names
        elif request == "Henry i need a pdf file opened" or request == "Henry can i open a pdf" or request == "Henry pdf":
            self.user_speak.Speak("alright, and what would the name be?")
            pdf_request = self.listen()
        else:
